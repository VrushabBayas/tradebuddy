...
from src.core.models import MarketData, OHLCV, TradingSignal, AnalysisResult, SignalAction, SignalStrength
...
class TestCompleteSystemIntegration:
    ...
    @pytest.mark.asyncio
    async def test_complete_system_flow(self, mock_market_data, mock_session_config, mock_analysis_result):
        ...
        try:
            ...
            with patch.object(cli_app.delta_client, 'get_market_data') as mock_get_data:
                ...
                with patch.object(cli_app.strategies[StrategyType.COMBINED], 'analyze') as mock_analyze:
                    ...
                    analysis_result = await strategy.analyze(market_data, mock_session_config)
                    ...
                    assert primary_signal.confidence >= mock_session_config.confidence_threshold
                    ...
                    logger.info("✅ Signal quality validation passed",
                    ...
                    if primary_signal.action == SignalAction.BUY:
                        stop_loss = entry_price * (1 - mock_session_config.stop_loss_pct / 100)
                        take_profit = entry_price * (1 + mock_session_config.take_profit_pct / 100)
                    else:
                        stop_loss = entry_price * (1 + mock_session_config.stop_loss_pct / 100)
                        take_profit = entry_price * (1 - mock_session_config.take_profit_pct / 100)
    ...
    async def test_system_performance(self, mock_market_data, mock_session_config):
        ...
        try:
            ...
            with patch.object(cli_app.delta_client, 'get_market_data') as mock_get_data:
                ...
                logger.info("✅ Performance test passed - handled 5 concurrent operations")
...
if __name__ == "__main__":
    ...
    asyncio.run(TestCompleteSystemIntegration().test_complete_system_flow(
        mock_market_data=None,
        mock_session_config=None,
