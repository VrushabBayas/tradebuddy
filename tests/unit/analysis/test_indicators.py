...
class TestTechnicalIndicators:
    ...
    def test_bollinger_bands(self, sample_ohlcv_data):
        ...
        bb = indicators.calculate_bollinger_bands(
            sample_ohlcv_data, period=20, std_dev=2
        )
        ...
        for i in range(len(sample_ohlcv_data)):
            if (
                bb["upper_band"][i] is not None
                and bb["middle_band"][i] is not None
                and bb["lower_band"][i] is not None
            ):
                assert (
                    bb["upper_band"][i] >= bb["middle_band"][i] >= bb["lower_band"][i]
                )
