...
from src.utils.environment import (
    EnvironmentValidator, ValidationResult, validate_environment,
...
class TestEnvironmentValidator:
    ...
    async def test_successful_validation(self):
        ...
        with patch.object(validator, '_check_python_version'), \
             patch.object(validator, '_check_required_packages'), \
             patch.object(validator, '_check_environment_variables'), \
             patch.object(validator, '_check_external_services'), \
             patch.object(validator, '_check_system_resources'):
    ...
    async def test_check_external_services_ollama_success(self, test_settings):
        ...
        with patch('aiohttp.ClientSession') as mock_session:
            ...
            assert validator.details["external_services"]["ollama"]["status"] == "✅ Connected"
    ...
    async def test_check_external_services_ollama_timeout(self, test_settings):
        ...
        with patch('aiohttp.ClientSession') as mock_session:
            ...
            assert "external_services" in validator.details
            assert "Timeout" in validator.details["external_services"]["ollama"]["status"]
    ...
    async def test_check_external_services_delta_exchange_success(self, test_settings):
        ...
        with patch('aiohttp.ClientSession') as mock_session:
            ...
            assert "external_services" in validator.details
            assert validator.details["external_services"]["delta_exchange"]["status"] == "✅ Connected"
    ...
    def test_check_system_resources_insufficient_memory(self):
        ...
        with patch('src.utils.environment.psutil', mock_psutil):
            ...
            assert "system_resources" in validator.details
            assert validator.details["system_resources"]["memory"]["sufficient"] is False
    ...
    def test_check_system_resources_without_psutil(self):
        ...
        with patch('src.utils.environment.psutil', None):
            with patch('importlib.import_module', side_effect=ImportError):
                ...
                assert "system_resources" in validator.details
                assert "psutil not available" in validator.details["system_resources"]["memory"]["status"]
                assert any("psutil not installed" in warning for warning in validator.warnings)
...
class TestValidateEnvironmentFunction:
    ...
    async def test_validate_environment_success(self):
        """Test successful environment validation."""
        with patch.object(EnvironmentValidator, 'validate_environment') as mock_validate:
...
class TestEnvironmentValidatorIntegration:
    ...
    async def test_full_validation_with_mocked_dependencies(self, test_settings):
        ...
        with patch('sys.version_info', (3, 11, 0)), \
             ...
             patch('src.utils.environment.settings', test_settings):
...
class TestEnvironmentValidatorEdgeCases:
    ...
    async def test_validator_with_http_error_responses(self, test_settings):
        ...
        with patch('aiohttp.ClientSession') as mock_session:
            ...
            assert "external_services" in validator.details
            assert "HTTP 500" in validator.details["external_services"]["ollama"]["status"]
    ...
    def test_validator_handles_missing_model_in_response(self, test_settings):
        ...
        async def run_test():
            with patch('aiohttp.ClientSession') as mock_session:
                ...
                assert "external_services" in validator.details
                assert not validator.details["external_services"]["ollama"]["has_required_model"]
                assert any("Required Ollama model" in warning for warning in validator.warnings)
