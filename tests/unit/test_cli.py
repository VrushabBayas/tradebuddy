...
class TestCLIApplication:
    ...
    async def test_configure_risk_parameters_invalid_then_valid(self):
        ...
        with patch.object(Prompt, 'ask', side_effect=[
            "-1.0", "5.0", "2.0",  # Invalid stop loss
            "2.5", "5.0", "2.0"    # Valid values
        ]):
            with patch.object(app, 'configure_risk_parameters', 
                             side_effect=[
                                 app.configure_risk_parameters(),  # First call (invalid)
                                 {"stop_loss": 2.5, "take_profit": 5.0, "position_size": 2.0}  # Second call (valid)
                             ]):
                risk_params = await app.configure_risk_parameters()
    ...
    async def test_configure_session_complete(self):
        ...
        with patch.object(app, 'select_symbol', return_value=Symbol.BTCUSDT), \
             patch.object(app, 'select_timeframe', return_value=TimeFrame.ONE_HOUR), \
             patch.object(app, 'configure_risk_parameters', 
                         return_value={"stop_loss": 2.5, "take_profit": 5.0, "position_size": 2.0}), \
             patch.object(app, 'display_configuration_summary'):
...
class TestCLIApplicationFlow:
    ...
    async def test_full_session_flow_exit_immediately(self):
        ...
        with patch.object(app, 'display_welcome'), \
             patch.object(app, 'select_strategy', return_value=None), \
    ...
    async def test_full_session_flow_single_analysis(self):
        ...
        with patch.object(app, 'display_welcome'), \
             patch.object(app, 'select_strategy', return_value=StrategyType.COMBINED), \
             patch.object(app, 'configure_session', return_value=MagicMock()), \
             patch.object(app, 'run_analysis_session'), \
             patch.object(app, 'ask_continue', return_value=False), \
             patch.object(app, 'display_goodbye'):
    ...
    async def test_full_session_flow_multiple_analyses(self):
        ...
        with patch.object(app, 'display_welcome'), \
             ...
             patch.object(app, 'configure_session', return_value=MagicMock()), \
             patch.object(app, 'run_analysis_session'), \
             patch.object(app, 'ask_continue', side_effect=[True, False]), \
             patch.object(app, 'display_goodbye'):
    ...
    async def test_session_flow_with_keyboard_interrupt(self):
        ...
        with patch.object(app, 'display_welcome'), \
             patch.object(app, 'select_strategy', side_effect=KeyboardInterrupt), \
    ...
    async def test_session_flow_with_exception_continue(self):
        ...
        with patch.object(app, 'display_welcome'), \
             patch.object(app, 'select_strategy', side_effect=[Exception("Test error"), StrategyType.COMBINED]), \
             patch.object(app, 'configure_session', return_value=MagicMock()), \
             patch.object(app, 'run_analysis_session'), \
             patch.object(app, 'ask_continue_after_error', return_value=True), \
             patch.object(app, 'ask_continue', return_value=False), \
             patch.object(app, 'display_goodbye'), \
    ...
    async def test_session_flow_with_exception_exit(self):
        ...
        with patch.object(app, 'display_welcome'), \
             patch.object(app, 'select_strategy', side_effect=Exception("Test error")), \
             patch.object(app, 'ask_continue_after_error', return_value=False), \
             patch.object(app, 'display_goodbye'), \
             patch.object(app.console, 'print'):
...
class TestCLIErrorHandling:
    ...
    async def test_run_handles_general_exception_development_mode(self, test_settings):
        ...
        with patch.object(app, 'display_welcome'), \
             ...
             patch.object(app, 'display_goodbye'), \
             patch.object(app.console, 'print') as mock_print, \
             patch('src.cli.main.settings', test_settings):
    ...
    async def test_run_handles_general_exception_production_mode(self):
        ...
        with patch.object(app, 'display_welcome'), \
             ...
             patch.object(app, 'display_goodbye'), \
             patch.object(app.console, 'print') as mock_print, \
             patch('src.cli.main.settings', mock_settings):
