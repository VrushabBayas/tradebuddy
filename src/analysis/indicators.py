...
class TechnicalIndicators:
    ...
    def calculate_ema(self, data: List[OHLCV], period: int) -> List[float]:
        ...
        logger.debug(f"EMA calculation completed", period=period, values_count=len(ema_values))
        return ema_values
    ...
    def detect_support_resistance(
        ...
        for level_price, touches in resistance_levels:
            level = SupportResistanceLevel(
                ...
                is_support=False,
                touches=touches,
                last_touch=data[-1].timestamp  # Simplified - would need actual last touch
            )
            levels.append(level)
    ...
    def calculate_rsi(self, data: List[OHLCV], period: int = 14) -> List[Optional[float]]:
        """
    ...
    def comprehensive_analysis(self, data: List[OHLCV]) -> Dict[str, Any]:
        ...
        logger.info("Performing comprehensive technical analysis", data_points=len(data))
        ...
        try:
            ...
            confidence_ratio = dominant_count / total_signals if total_signals > 0 else 0
            ...
            confidence_score = min(10, max(1, int(
                (confidence_ratio * 0.5 + trend_factor * 0.3 + crossover_factor * 0.2) * 10
            )))
