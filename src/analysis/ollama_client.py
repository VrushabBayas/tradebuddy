...
from src.core.constants import OllamaConstants, ErrorCodes
...
Provide a clear, actionable trading signal with specific entry/exit levels and detailed reasoning."""
    ...
    def _add_ema_analysis(self, analysis: Dict[str, Any]) -> str:
        """Add EMA-specific analysis to prompt."""
        ...
        if hasattr(ema_data, 'ema_9'):
            ...
            is_golden_cross = ema_data.get('is_golden_cross', False) if ema_data else False
            crossover_strength = ema_data.get('crossover_strength', 0) if ema_data else 0
...
Average Volume: {volume_data.get('average_volume', 0):,.1f}
...
Only provide BUY/SELL signals when both strategies align. Use NEUTRAL when strategies conflict."""
    ...
    def _parse_trading_signals(
        ...
        try:
            ...
            confidence_matches = re.findall(confidence_pattern, ai_response, re.IGNORECASE)
            entry_matches = re.findall(entry_pattern, ai_response, re.IGNORECASE)
            stop_loss_matches = re.findall(stop_loss_pattern, ai_response, re.IGNORECASE)
            take_profit_matches = re.findall(take_profit_pattern, ai_response, re.IGNORECASE)
