...
class CombinedStrategy(BaseStrategy):
    ...
    async def analyze(
        ...
        try:
            ...
            filtered_result = self._add_combined_context(
                filtered_result, technical_analysis
            )
            ...
            logger.info(
                ...
                strategy_alignment=technical_analysis.get("combined_context", {}).get(
                    "strategy_alignment"
                ),
                confirmation_level=technical_analysis.get("combined_context", {}).get(
                    "confirmation_level"
                ),
    ...
    def _enhance_combined_analysis(
        self, analysis: dict, market_data: MarketData
    ) -> dict:
        """
        ...
        try:
            ...
            # Analyze S/R signals
            sr_signal = self._analyze_sr_signal(
                sr_levels, market_data.current_price, volume_analysis
            )

            # Determine strategy alignment
            strategy_alignment = self._determine_strategy_alignment(
                ema_signal, sr_signal
            )
            ...
            combined_signal = self._generate_combined_signal(
                ema_signal, sr_signal, confirmation_level
            )
            ...
            analysis["combined_context"] = {
                ...
                "overall_conviction": self._calculate_overall_conviction(
                    ema_signal, sr_signal, volume_analysis
                ),
            }
    ...
    def _analyze_ema_signal(self, ema_crossover, volume_analysis: dict) -> dict:
        ...
        if volume_ratio > TradingConstants.STRONG_VOLUME_THRESHOLD:
            adjusted_strength = min(
                10, adjusted_strength + TradingConstants.VOLUME_CONFIDENCE_BONUS
            )
        ...
        return {
            ...
            > TradingConstants.VOLUME_CONFIRMATION_THRESHOLD,
        }

    def _analyze_sr_signal(
        self, sr_levels: list, current_price: float, volume_analysis: dict
    ) -> dict:
        ...
        if nearest_support:
            ...
            if abs(current_price - support_price) <= tolerance:
                ...
                confidence = min(
                    ...
                    strength
                    + (
                        TradingConstants.VOLUME_CONFIDENCE_BONUS
        ...
        if nearest_resistance:
            ...
            if abs(current_price - resistance_price) <= tolerance:
                ...
                if direction == "neutral":
                    ...
                    confidence = max(
                        1, confidence - TradingConstants.VOLUME_CONFIDENCE_BONUS
                    )
    ...
    def _calculate_confirmation_level(
        ...
        avg_strength = (
            ema_signal.get("strength", 0) + sr_signal.get("strength", 0)
        ) / 2
        confirmation += (avg_strength / 10) * (
            10 * TradingConstants.SIGNAL_STRENGTH_WEIGHT
        )
    ...
    def _generate_combined_signal(
        self, ema_signal: dict, sr_signal: dict, confirmation_level: int
    ) -> dict:
        ...
        base_confidence = (
            ema_signal.get("confidence", 1) + sr_signal.get("confidence", 1)
        ) / 2
        ...
        return {
            ...
            "strength": "strong"
            if confirmation_level >= TradingConstants.STRONG_CONFIRMATION_LEVEL
            else "moderate"
            if confirmation_level >= TradingConstants.MODERATE_CONFIRMATION_LEVEL
            else "weak",
        }
    ...
    def _assess_volume_strength(self, volume_analysis: dict) -> str:
        ...
        if (
            volume_ratio > TradingConstants.VERY_STRONG_VOLUME_THRESHOLD
    ...
    def _calculate_overall_conviction(
        self, ema_signal: dict, sr_signal: dict, volume_analysis: dict
    ) -> int:
    ...
    def _apply_combined_filtering(
        ...
        min_confidence = max(
            session_config.confidence_threshold,
            TradingConstants.MODERATE_CONFIRMATION_LEVEL,
        )
        ...
        for signal in analysis_result.signals:
            if (
                ...
                and confirmation_level >= TradingConstants.MODERATE_CONFIRMATION_LEVEL
            ):  # Require decent confirmation
    ...
    def _generate_combined_context_text(self, combined_context: dict) -> str:
        ...
        lines.append(
            f"EMA Signal: {ema_signal.get('direction', 'neutral').title()} (Confidence: {ema_signal.get('confidence', 0)}/10)"
        )
        lines.append(
            f"S/R Signal: {sr_signal.get('direction', 'neutral').title()} (Confidence: {sr_signal.get('confidence', 0)}/10)"
        )
        lines.append(
            f"Strategy Alignment: {combined_context.get('strategy_alignment', 'unknown').replace('_', ' ').title()}"
        )
        lines.append(
            f"Confirmation Level: {combined_context.get('confirmation_level', 0)}/10"
        )
        lines.append(
            f"Volume Strength: {combined_context.get('volume_strength', 'unknown').replace('_', ' ').title()}"
        )
        lines.append(
            f"Overall Conviction: {combined_context.get('overall_conviction', 0)}/10"
        )

        combined_signal = combined_context.get("combined_signal", {})
        lines.append(
            f"Combined Signal: {combined_signal.get('direction', 'neutral').title()} ({combined_signal.get('strength', 'weak').title()})"
        )
