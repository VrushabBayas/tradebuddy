...
class CombinedStrategy(BaseStrategy):
    ...
    def _analyze_ema_signal(self, ema_crossover, volume_analysis: dict) -> dict:
        ...
        is_golden_cross = self._get_value(ema_crossover, 'is_golden_cross', False)
        crossover_strength = self._get_value(ema_crossover, 'crossover_strength', 0)
        ...
        if volume_ratio > 1.3:
            adjusted_strength = min(10, adjusted_strength + 2)
    ...
    def _analyze_sr_signal(self, sr_levels: list, current_price: float, volume_analysis: dict) -> dict:
        ...
        if nearest_support:
            ...
            support_strength = self._get_value(nearest_support, 'strength', 0)
            
            if abs(current_price - support_price) <= tolerance:
                # Near support - potential bounce
                direction = 'bullish'
                strength = support_strength
                confidence = min(10, strength + (2 if volume_ratio > 1.2 else 0))
        
        if nearest_resistance:
            resistance_price = self._get_value(nearest_resistance, 'level', 0)
            resistance_strength = self._get_value(nearest_resistance, 'strength', 0)
    ...
    def _calculate_confirmation_level(
        ...
        confirmation += (avg_strength / 10) * 3.0
    ...
    def _generate_combined_signal(self, ema_signal: dict, sr_signal: dict, confirmation_level: int) -> dict:
        ...
        return {
            ...
            'strength': 'strong' if confirmation_level >= 8 else 'moderate' if confirmation_level >= 6 else 'weak'
        }
    ...
    def _apply_combined_filtering(
        ...
        min_confidence = max(session_config.confidence_threshold, 6)
        
        # Filter signals based on confirmation level and confidence
        filtered_signals = []
        for signal in analysis_result.signals:
            if (signal.confidence >= min_confidence and 
                confirmation_level >= 6):  # Require decent confirmation
