...
class EnvironmentValidator:
    ...
    async def validate_environment(self) -> ValidationResult:
        ...
        try:
            ...
            return ValidationResult(
                is_valid=True, warnings=self.warnings, details=self.details
            )
        ...
        for package in required_packages:
            try:
                ...
                if package == "yaml":
                    ...
                    installed_packages[package] = getattr(
                        yaml, "__version__", "unknown"
                    )
                elif package == "dotenv":
                    import dotenv

                    installed_packages[package] = getattr(
                        dotenv, "__version__", "unknown"
                    )
                else:
    ...
    def _check_environment_variables(self):
        ...
        critical_vars = ["DELTA_EXCHANGE_API_URL", "OLLAMA_API_URL", "OLLAMA_MODEL"]

        optional_vars = ["REDIS_URL", "DATABASE_URL", "LOG_LEVEL", "PYTHON_ENV"]
    ...
    async def _check_external_services(self):
        ...
        try:
            ...
            self.warnings.append(f"Cannot connect to Ollama API: {str(e)}")
        ...
        try:
            async with aiohttp.ClientSession(
                ...
                async with session.get(
                    f"{settings.delta_exchange_api_url}/v2/products"
                ) as response:
                    if response.status == 200:
            ...
            self.warnings.append(f"Cannot connect to Delta Exchange API: {str(e)}")
